...
useEffect(() => {
  ...
  function selecionaCurso(cursoSelect,horarioSelect, nome){
    setCheckedCurso(cursoSelect); 
    setCargaHoraria(horarioSelect);
    setCurso(nome)
  }
 
  function selecionaTeacher(codigo, nome){
    setCheckedTeacher(codigo)
    setProfessor(nome)
  }
  ...
  const renderizaCurso = ({item})=>{
    ...
    return(
      <View style={styles.containerCurso}>
              <RadioButton value={item.codigo} status={ checkedCurso ===  (item.codigo)? 'checked' : 'unchecked' } onPress={() => selecionaCurso(item.codigo,item.carga_horaria, item.nome)} style={styles.buttonRadio}/>
      ...
      const renderizaTeacher = ({item})=>{
        return(
          <View style={styles.containerCurso}>
                  <RadioButton value={item.codigo} status={ checkedTeacher ===  (item.codigo)? 'checked' : 'unchecked' } onPress={() => selecionaTeacher(item.codigo,item.nome)} style={styles.buttonRadio}/>
            <Text  style={styles.text}>{item.nome}</Text>
          </View>
          );
          }
